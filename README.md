...
- [常见问题](#常见问题)
- [.env.example 说明](#envenv-example-说明)
- [许可证](#许可证)
...
> ⚠️ **重要提示：构建阶段 vs 运行阶段**
>
> - **构建阶段**（GitHub Actions / Docker build）必须使用 **Modrinth 官方 API** (`api.modrinth.com`)
> - **运行阶段**（生产环境）使用你自己的域名（如 `api.bbsmc.org.cn`）
> - 仓库中的 `.env.example` 已默认配置为 Modrinth 官方地址，确保编译不会失败
> - 实际部署时请复制为 `.env` 并替换为你自己的域名

### `.env.example` 与 `.env` 的关系

| 文件           | 用途                                       | 是否跟踪到 Git                  |
| -------------- | ------------------------------------------ | ------------------------------- |
| `.env.example` | 环境变量模板，构建时使用 Modrinth 官方地址 | ✅ 是（已推送到仓库）           |
| `.env`         | 实际运行配置，使用你自己的域名和密钥       | ❌ 否（已在 .gitignore 中忽略） |

```bash
# 从模板创建实际配置文件
cp apps/frontend/.env.example apps/frontend/.env
cp apps/labrinth/.env.example apps/labrinth/.env

# 然后编辑 .env，将 Modrinth 官方地址替换为你自己的域名
```

### 前端环境变量 (`apps/frontend/.env`)

> 构建时 `.env.example` 使用 `https://api.modrinth.com/v2/` 防止编译失败
> 运行时 `.env` 应替换为你自己的 API 域名

```bash
# 构建阶段（.env.example 默认值，防止编译失败）
BASE_URL=https://api.modrinth.com/v2/
BROWSER_BASE_URL=https://api.modrinth.com/v2/

# 运行阶段（.env 实际值，替换为你自己的域名）
...
> 构建时 `.env.example` 使用 Modrinth 官方地址防止编译失败
> 运行时 `.env` 应替换为你自己的域名
...
> ⚠️ **最常见原因**：构建时使用了自定义域名（如 `api.bbsmc.org.cn`），而 GitHub Actions 环境无法访问该域名导致编译失败。
>
> **解决方案**：确保 `.env.example` 使用 Modrinth 官方 API `https://api.modrinth.com/v2/`，只有运行时的 `.env` 才使用自定义域名。

```bash
# 确认 .env.example 使用 Modrinth 官方地址
...
> ⚠️ 注意：`.env.example` 保持使用 Modrinth 官方地址（用于构建），只修改 `.env`（用于运行）
...
仓库包含两个 `.env.example` 环境变量模板文件，**已推送到 GitHub 仓库**，用于确保构建阶段不会因域名问题导致编译失败。

### 文件位置

| 文件     | 路径                         | 用途                              |
| -------- | ---------------------------- | --------------------------------- |
| 前端模板 | `apps/frontend/.env.example` | 前端构建时的 API 地址配置         |
| 后端模板 | `apps/labrinth/.env.example` | 后端构建时的站点/CDN/API 地址配置 |

### 为什么构建时使用 Modrinth 官方地址？

在 GitHub Actions 或 Docker 构建阶段，构建环境**无法访问自定义域名**（如 `api.bbsmc.org.cn`），如果 `.env.example` 使用了自定义域名，会导致：

- ❌ 前端构建时 SSR 预渲染无法访问 API，导致编译失败
- ❌ 后端编译时环境变量校验失败

因此 `.env.example` 统一使用 **Modrinth 官方地址**：

| 变量               | `.env.example`（构建用）       | `.env`（运行用）                 |
| ------------------ | ------------------------------ | -------------------------------- |
| `BASE_URL`         | `https://api.modrinth.com/v2/` | `https://api.bbsmc.org.cn/v2/`   |
| `BROWSER_BASE_URL` | `https://api.modrinth.com/v2/` | `https://api.bbsmc.org.cn/v2/`   |
| `SITE_URL`         | `https://modrinth.com`         | `https://bbsmc.org.cn`           |
| `CDN_URL`          | `https://cdn.modrinth.com`     | `https://cdn.bbsmc.org.cn/bbsmc` |
| `SELF_ADDR`        | `https://api.modrinth.com`     | `https://api.bbsmc.org.cn`       |
...
创建后编辑 `.env` 文件，将 Modrinth 官方地址替换为你自己的域名，并填写实际的数据库密码、密钥等敏感信息。
...
- 允许 `.env.example` 文件被跟踪（确保模板推送到仓库）

```
# .gitignore 相关规则
.env                          # 忽略实际配置
apps/frontend/.env            # 忽略前端实际配置
apps/labrinth/.env            # 忽略后端实际配置
!.env.example                 # 允许模板文件
!apps/frontend/.env.example   # 允许前端模板
!apps/labrinth/.env.example   # 允许后端模板
```
